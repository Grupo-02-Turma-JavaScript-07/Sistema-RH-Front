...
import { FaHome, FaUser, FaChevronLeft, FaChevronRight, FaBars } from "react-icons/fa"
import { IoIosArrowDown, IoIosArrowUp } from "react-icons/io"
import { Link, useLocation } from "react-router-dom"

interface SidebarProps {
  minimizado: boolean;
  setMinimizado: (value: boolean) => void;
}

function Sidebar({ minimizado, setMinimizado }: SidebarProps) {
  ...
  return (
    <>
      <button
        ...
        className="md:hidden fixed top-4 left-4 z-50 bg-white p-2 rounded-md shadow border border-gray-200"
      >
        <FaBars />
      </button>

      <div className={`fixed top-0 left-0 h-full z-40 transition-all duration-300 ${mobileAberto ? "translate-x-0" : "-translate-x-full"} md:translate-x-0 flex`}>
        <aside className={`min-h-screen bg-white border-r border-gray-200 text-sm font-medium relative transition-all duration-300 ${minimizado ? "w-16" : "w-60"}`}>
          <button
            ...
            className="hidden md:block absolute -right-3 top-2 bg-white border border-indigo-100 cursor-pointer rounded-full p-1 shadow hover:bg-gray-100"
          >
          ...
          <div className={`pt-4 pb-3 ${minimizado ? "text-center" : "px-5"} font-zain text-xl font-semibold tracking-wide text-gray-800`}>
            {!minimizado && (
              <Link to='/'>
                <span className="text-cyan-400 font-marko text-xl whitespace-nowrap">[Flow G2 RH]</span>
              </Link>
            )}
          </div>
          ...
          <Link to="/perfil">
            <div className={`relative flex items-center gap-2 px-4 py-2 mb-1 rounded-md cursor-pointer ${inicioEstaAtivo ? "bg-indigo-100 text-indigo-900" : "text-gray-700 hover:bg-gray-100"}`}>
              {inicioEstaAtivo ? <span className="absolute left-0 top-1 bottom-1 w-1 bg-indigo-600 rounded-r-md" /> : null}
              <FaHome size={16} />
        ...
        <div className="hidden md:block w-px bg-indigo-100 h-screen" />
      </div>
    </>
  )
}
